...
public class Attendancerecord extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException{
		...
		String Classhour = req.getParameter("Classhour");
		...
		try {//判断数据库有没这个用户名
			...
			//String sql="SELECT * FROM SignRecord WHERE name='"+name+"';";
				...
				//String sql = "SELECT COUNT(username) AS isExist FROM user WHERE username='" + username + "';";
			...
			if(results.getRow()==1) {//第一个数据
				...
				String sql1="INSERT INTO SignRecord(Stuid,name,Signtime,Classhour,SignDate)VALUES('"+Stuid+"','"+name+"','"+Signtime+"','"+Classhour+"','"+SignDate+"');";
